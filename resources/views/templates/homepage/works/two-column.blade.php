...
$singleWork = (count($works) === 1);
@endphp

@unless ( empty( $works ) )
  <section class="{{ $singleWork ? 'tpl-homepage-works-full-width' : 'tpl-homepage-works-two-column' }} s-sequence-group">
    <div class="{{ $singleWork ? '' : 'container-fluid' }}">
      <div class="{{ $singleWork ? '' : 'grid grid-cols-1 gap-11 md:grid-cols-2 md:gap-10' }}">
              ...
              @if( !empty( $work['image'] || !empty($work['use_video']) ) )
                <div class="work-item-image-wrapper cursor-view {{$work['cursor_icon']}} s-sequenced">
                  @if( $work['use_video'] )
                    ...
                    @endswitch
                  @endif
                  @if ( empty($work['video_url']) && !empty($work['image']) )
                    <img src="{{ $work['image'] }}"
